...
float Latest_ERR[5] = {0,0,0,0,0};//[0]:Update STATUS [1:]:Error values
float pid_0o;
float vlc_0o;
/* USER CODE END PV */
...
int main(void)
{
  ...
  DCI_Targ_Modify_Offset(&DCIController, 15, 0);
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
  ...
  if(htim -> Instance == TIM7){
    for(int i=0;i<4;i++){
      ...
      Latest_ERR[i+1] = error_calc(PID_Devices_Handlers[i]);
...
void PID_CON_Update(void){
  if((uint16_t)Latest_ERR[0] == RESET){
    /*
    __HAL_TIM_GetCompare(&htim2, TIM_CHANNEL_1);
    __HAL_TIM_GetCompare(&htim2, TIM_CHANNEL_2);
    __HAL_TIM_GetCompare(&htim2, TIM_CHANNEL_3);
    __HAL_TIM_GetCompare(&htim2, TIM_CHANNEL_4);
    ...
    */
    ...
    for(int i = 0; i <4; i++){
      channel_id = i*4;
      calc_res = __HAL_TIM_GetCompare(&htim2, channel_id) + Latest_ERR[i+1];
